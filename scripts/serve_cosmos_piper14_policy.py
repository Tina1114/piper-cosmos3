...
from piper_cosmos.deployment.cosmos_piper14_policy import CosmosPiper14PolicyConfig  # noqa: E402
from piper_cosmos.deployment.cosmos_piper14_policy_server import serve_cosmos_piper14_policy  # noqa: E402
from piper_cosmos.cosmos3.local_hf_assets import (  # noqa: E402
    WAN_VAE_REPOSITORY,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--algo",
        default="rtn",
        help="Quantization algorithm expected in quantization_config.json (default: rtn).",
    )
    parser.add_argument(
        "--backend",
        default="fakequant",
        help="Quantized checkpoint runtime kernel/backend (default: fakequant).",
    )
    parser.add_argument(
        "--require-quantized-checkpoint",
        action="store_true",
        help="Fail before model loading when --checkpoint has no quantization_config.json.",
    )
...
def main() -> None:
    ...
    config = CosmosPiper14PolicyConfig(
        ...
        require_quantized_checkpoint=args.require_quantized_checkpoint,
        host=args.host,
        port=args.port,
